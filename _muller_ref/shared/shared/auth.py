...
async def require_admin_or_service(
    ...
    """Accept either a session-based admin user or an internal service API key.

    The service key is sent via the X-Internal-Key header and must match
    settings.INTERNAL_API_KEY. Used for scraper→catalog sync calls.
    """
    internal_key = request.headers.get("X-Internal-Key")
    if internal_key and settings.INTERNAL_API_KEY and internal_key == settings.INTERNAL_API_KEY:
        return {"user_id": "internal-service", "role": "admin", "username": "service"}
    return await require_admin(request, redis)
